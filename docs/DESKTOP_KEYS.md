...
|------|----------------------|
| **Local dev** | `timeflow-desktop-app/.env` (gitignored) |
| **Installed .exe** | HTTP fetch of `https://timeflow.mechlintech.com/desktop-config.json` at startup |
...
Electron preload: `getSupabaseConfig()` → env first (dev), else remote JSON.
...
3. Keep local `.env` only for developers.
4. Confirm: `https://timeflow.mechlintech.com/desktop-config.json` returns JSON with `supabaseUrl` + `supabasePublishableKey`.
